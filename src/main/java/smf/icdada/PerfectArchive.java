...
public class PerfectArchive {
    public static void single() {
        Log.v("请输入拓维userId");
        int userId = smfScanner.Int(true, "^\\d{8,}$");
        Log.v("""
                请输入需要执行的功能
                [1] 全神器刷取
                [2] 全植物刷取
    ...
    public static void measure() {
        ...
        for (int user : users) {
            ...
            executorService.submit(() -> {
                switch (index) {
                    case 1 -> rewardArtifactBrush(user);
                    case 2 -> rewardPlantBrush(user);
                    case 3 -> rewardCustomBrush(user);
                }
    ...
    private static void rewardArtifactBrush(int userId) {
        ...
        while (true) {
            ...
            try {
                ...
                if (v316.isValid(0)) {
                    ...
                    while (true) {
                        int artifactUk = ++uk;
                        Future<String> futureV302Artifact = executor.submit(() -> getResponseBody(V302, userId, rewardPieces(artifactUk), artifactUk));
                        try {
                            ...
                            if (v302.isValid(11102)) {
                                ...
                                while (i != 1) {
                                    uk++;
                                    int plantUk = uk;
                                    Future<String> futureV302Plant = executor.submit(() -> getResponseBody(V302, userId, rewardArtifacts(plantUk), plantUk));
                                    try {
    ...
    private static void rewardPlantBrush(int userId) {
        Check.V316 v316 = new Check.V316();
        Check.V302 v302 = new Check.V302();
    ...
    private static void rewardCustomBrush(int userId) {
        Check.V316 v316 = new Check.V316();
        Check.V302 v302 = new Check.V302();
        ProgressBar progressBar = new ProgressBar("账号: " + userId + " 刷取进度 || ", 3, true);
        refresh(userId);
                        ...
                        try {
                            ...
                            if (v302.isValid(11102)) {
                                ...
                                while (i != 1) {
                                    uk++;
                                    int customUk = uk;
                                    Future<String> futureV302Custom = executor.submit(() -> getResponseBody(V302, userId, rewardCustoms(customUk), customUk));
                                    try {
            ...
            try {
                ...
                if (v316.isValid(0)) {
                    ...
                    for (int[] range : ranges) {
                        ...
                        Future<String> futureV302 = executor.submit(()->getResponseBody(V302,userId,array,finalUk));
                        try {
